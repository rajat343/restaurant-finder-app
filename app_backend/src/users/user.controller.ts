...
router.put("/update_user", [isUserLoggedIn], async (req, res) => {
	try {
...
router.put("/make_admin", [], async (req: Request, res: Response) => {
	try {
		const userId = req.body.user_id;
		const updateQuery = { is_admin: true };
		const updatedUser = await UsersService.updateSingleUser(
			{ _id: userId },
			updateQuery
		);
		...
		res.status(500).json({
			error: `Error while making user admin: ${err.message}`,
		});
	}
});
